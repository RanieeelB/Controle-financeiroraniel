    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center min-h-[360px] sm:min-h-[400px] text-on-surface-variant gap-md px-4 text-center">
          ...
          <h2 className="font-h1 text-[24px] sm:text-[32px] font-semibold text-on-surface">Nenhuma meta cadastrada</h2>
          ...
          <button
            ...
            className="mt-md font-label-md text-[14px] font-semibold bg-primary text-on-primary px-lg py-sm rounded-full hover:bg-primary-container transition-all shadow-[0_0_15px_rgba(117,255,158,0.2)] flex items-center justify-center gap-sm min-h-11 w-full sm:w-auto"
          >
    ...
    <div className="space-y-lg lg:space-y-xl min-w-0">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-sm sm:gap-lg">
        <div className="bg-surface-container border border-outline-variant rounded-xl p-md sm:p-lg relative overflow-hidden min-w-0 sm:col-span-2 lg:col-span-1">
          ...
          <span className="font-numeral-lg text-[24px] min-[390px]:text-[28px] sm:text-[32px] text-on-surface break-words">R$ {fmt(totalSaved)}</span>
        ...
        <div className="bg-surface-container border border-outline-variant rounded-xl p-md sm:p-lg min-w-0">
          ...
          <span className="font-numeral-lg text-[24px] min-[390px]:text-[28px] sm:text-[32px] text-on-surface break-words">R$ {fmt(totalTarget - totalSaved)}</span>
        </div>
      </section>

      {/* Goals Grid */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg min-w-0">
        {goals.map(goal => {
          ...
          return (
            <div key={goal.id} className="bg-surface-container border border-outline-variant rounded-xl p-md sm:p-lg relative overflow-hidden group hover:border-primary/50 transition-colors min-w-0">
              ...
              <div className="flex justify-between gap-md">
                ...
                <span className="text-[14px] text-on-surface text-right">R$ {fmt(goal.target_amount)}</span>
              </div>
              {goal.deadline && (
