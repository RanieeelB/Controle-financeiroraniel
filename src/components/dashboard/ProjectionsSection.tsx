  ...
  return (
    <section className="space-y-md mt-xl">
      <div className="flex items-center gap-sm">
        ...
        <h3 className="font-h2 text-[24px] font-semibold text-on-background">Projeções Financeiras</h3>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-md">
        {projections.map((proj) => (
          <div key={proj.monthKey} className="glass-card rounded-2xl p-lg flex flex-col gap-md border border-outline-variant/30 hover:border-primary/30 transition-all group">
            ...
            <div>
              ...
              <p className="font-numeral-lg text-[28px] font-bold text-on-surface">R$ {fmt(proj.total)}</p>
            </div>

            <div className="space-y-xs">
              <div className="flex justify-between text-[13px]">
                <span className="text-on-surface-variant">Cartão:</span>
                <span className="font-medium text-primary">R$ {fmt(proj.breakdown.creditCards)}</span>
              </div>
              <div className="flex justify-between text-[13px]">
                <span className="text-on-surface-variant">Fixos + Aportes:</span>
                <span className="font-medium text-on-surface">R$ {fmt(proj.breakdown.fixedBills + proj.breakdown.investments)}</span>
              </div>
            </div>

            <button 
              onClick={() => setSelectedProjection(proj)}
              className="mt-sm w-full py-sm bg-surface-container-highest rounded-xl text-on-surface font-semibold hover:bg-primary hover:text-on-primary transition-all text-[14px]"
            >
              Ver detalhes
            </button>
          </div>
