...
export function CreditCardInvoices({ data }: CreditCardInvoicesProps) {
  ...
  return (
    <>
      <h3 className="font-h2 text-[24px] font-semibold text-on-background mb-xs">Faturas de cartão</h3>
      {data.map((card) => (
        <div key={card.id} className="glass-card rounded-xl p-md flex justify-between items-center">
          <div className="flex items-center gap-sm">
            ...
            <div>
              <p className="text-[14px] font-medium text-on-background">{card.name}</p>
